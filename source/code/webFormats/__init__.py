...
htmlPreviewDefault = f'''\
<h1>%(familyName)s %(styleName)s</h1>
<p>{string.ascii_uppercase}<br/>
...
class TTHAutoHintSettings(BaseWindowController):
    ...
    def __init__(self, parentWindow):
        ...
        self.w = Sheet((470, 480), parentWindow=parentWindow)
        ...
        y += 30
        self.w.tabs[1].html = self.html = CodeEditor((10, y, -10, 150), getExtensionDefault("%s.htmlPreview" % settingsIdentifier, htmlPreviewDefault), lexer="html")
        self.html.showLineNumbers(False)
        y += 160
        self.w.tabs[1].globalCssText = TextBox((10, y, 100, 22), "CSS Style:")
        y += 30
        self.w.tabs[1].globalCss = self.globalCss = CodeEditor((10, y, -10, -10), getExtensionDefault("%s.globalCSSPreview" % settingsIdentifier, cssPreviewDefault), lexer="css")
    ...
    def resetCallback(self, sender):
        if self.w.tabs.get() == 0:
            ...
            setExtensionDefault("%s.globalCSSPreview" % settingsIdentifier, cssPreviewDefault)
            self.globalCss.set("")
...
class WebFormats(Group):
    ...
    def __init__(self, posSize, controller):
        ...
        self.webSuffix = EditText((middle + 15, y, 100, 22),
            getExtensionDefault("%s.webSuffix" % settingsIdentifier, "_web"),
            callback=self.saveDefaults)
        y += 30

        self.convert = Button((-100, -35, -10, 22), "Generate", callback=self.convertCallback)
        self.settings = ImageButton((-130, -33, 20, 20), bordered=False, imageNamed=NSImageNameSmartBadgeTemplate, callback=self.settingsCallback)
